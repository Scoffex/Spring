...
public class ClientBankAccountServiceImpl implements IclientBankAccountService {
	...
	public ResponseFromApi save(String panCode) throws GenericError {
		...
		try {
			...
			StringBuilder sb = new StringBuilder(
					"ERROR IN SAVE METHOD FOR BANK ACCOUNT - PAN CODD NOT PRESENT IN THE SYSTEM: ").append(e);
	...
	public BankAccount createBankAccount(ClientBank client) throws GenericError {
		...
		try {
			...
			StringBuilder sb = new StringBuilder("ERROR SAVING IN BANK ACCOUNT TABLE: ").append(e);
			logger.error(sb.toString());
			throw new GenericError(sb.toString());
		}
		return ba;
	...
	public ResponseFromApi findAll() throws GenericError {
		...
		try {
			...
			StringBuilder sb = new StringBuilder("ERROR FIND ALL BANK ACCOUNT: ").append(e);
			logger.error(sb.toString());
			throw new GenericError(sb.toString());
		}
		return response;
	...
	public ResponseFromApi delete(String iban) throws GenericError {
		try {
			...
			StringBuilder sb = new StringBuilder("ERROR DELETING ACCOUNT WITH IBAN ").append(iban)
					.append(" ENTITY NOT FOUND");
			logger.error(sb.toString());
			throw new GenericError(sb.toString());
		}
	...
	public ResponseFromApi findByPanCode(String panCode) throws GenericError {
		...
		try {
			...
			List<BankAccount> list = repo.findAll().stream().filter(x -> x.getClientBank().getId() == cb.getId())
					.toList();
			response.setBankAccount(ConversionAnagraficAccount.fromListDomainBanotoListDtoBank(list));
			response.setMessage(Constants.ACCOUNT_FOUND);
			response.setStatudCode(HttpStatus.OK);
