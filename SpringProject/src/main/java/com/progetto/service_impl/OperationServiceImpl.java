...
public class OperationServiceImpl implements IoperationService {
	...
	@Autowired
	ClientBankAccountServiceImpl bankService;
		...
		try {
			...
			if (Constants.CHECK.equalsIgnoreCase(transaction.getStatus())) {
				...
				if (Constants.DEPOSIT.equalsIgnoreCase(transaction.getOperation())) {
					ba.setAmount(ba.getAmount() + transaction.getAmount());
				} else {
					ba.setAmount(ba.getAmount() - transaction.getAmount());
				}
				OperationDto operationDto = new OperationDto(atmCode, timeOperation, iban, transaction.getOperation());
